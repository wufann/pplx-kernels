...
AllToAllInterNode::AllToAllInterNode(
  ...
  numTokensBuffer = (uint64_t *)rocshmem::rocshmem_malloc(sizeof(uint64_t) * numLocalExperts * numDPGroups);
  PPLX_ASSERT(numTokensBuffer != nullptr, "failed to allocate numTokensBuffer");
  hipMemset(numTokensBuffer, 0, sizeof(uint64_t) * numLocalExperts * numDPGroups);

  numDispatchRecvBuffer =
      (uint64_t *)rocshmem::rocshmem_malloc(sizeof(uint64_t) * numLocalExperts * numDPGroups);
  PPLX_ASSERT(numDispatchRecvBuffer != nullptr, "failed to allocate numDispatchRecvBuffer");
  hipMemset(numDispatchRecvBuffer, 0, sizeof(uint64_t) * numLocalExperts * numDPGroups);

  combineSignalBuffer = (uint64_t *)rocshmem::rocshmem_malloc(sizeof(uint64_t) * maxNumTokens);
  PPLX_ASSERT(combineSignalBuffer != nullptr, "failed to allocate combineSignalBuffer");
  hipMemset(combineSignalBuffer, 0, sizeof(uint64_t) * maxNumTokens);

  combineSyncBuffer = (uint64_t *)rocshmem::rocshmem_malloc(sizeof(uint64_t) * worldSize);
  PPLX_ASSERT(combineSyncBuffer != nullptr, "failed to allocate combineSyncBuffer");
  hipMemset(combineSyncBuffer, 0, sizeof(uint64_t) * worldSize);

  // Buffers for dispatch.
  const size_t perTokenBytes =
      round_up<size_t>(hiddenDimBytes + hiddenDimScaleBytes + sizeof(uint32_t), 16);
  xDispatchIn = (std::byte *)rocshmem::rocshmem_malloc(maxNumTokens * perTokenBytes);
  PPLX_ASSERT(xDispatchIn != nullptr, "failed to allocate xDispatchIn");
  xDispatchOut = (std::byte *)rocshmem::rocshmem_malloc(maxBatchTokens * perTokenBytes);
  ...
  xCombineIn = (std::byte *)rocshmem::rocshmem_malloc(maxBatchTokens * hiddenDim * sizeof(float));
  PPLX_ASSERT(xCombineIn != nullptr, "failed to allocate xCombineIn");
  xCombineOut = (std::byte *)rocshmem::rocshmem_malloc(maxNumTokens * numExperts * hiddenDim * sizeof(float));
...
AllToAllInterNode::~AllToAllInterNode() {
  CUDACHECK(hipFree(numTokensPerDP));
  rocshmem::rocshmem_free(numTokensBuffer);
  rocshmem::rocshmem_free(numDispatchRecvBuffer);
  rocshmem::rocshmem_free(combineSignalBuffer);
  rocshmem::rocshmem_free(combineSyncBuffer);
  rocshmem::rocshmem_free(xDispatchIn);
  rocshmem::rocshmem_free(xDispatchOut);
  rocshmem::rocshmem_free(xCombineIn);
  rocshmem::rocshmem_free(xCombineOut);

  CUDACHECK(hipFree(sourceIndex));
  CUDACHECK(hipFree(sourceExpert));
  CUDACHECK(hipFree(sourceOffset));
  CUDACHECK(hipFree(sourceGroup));
